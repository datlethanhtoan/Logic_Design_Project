...
unsigned char FPC1020::WaitFpData(void)
{
    ...
    while (mySerial->available() <= 0 && outMode2 == 1){
        digitalWrite(2, HIGH);
    ...
    Serial.println("Read Return buffer success");
    end:
    if (rBuf_p == 0)
        return FALSE; 
    else
...
{
    //Serial.println("start check package");
    unsigned char flag = FALSE;
    unsigned char flagwait = WaitFpData();
    if (flagwait == FALSE){
        return flag;
    }
    else if(flagwait == ACK_TIMEOUT){
       //digitalWrite(4, HIGH);
       //Serial.println("out");
       return ACK_TIMEOUT;
    }
    ...
    case CMD_ENROLL3:
        ...
        // else if (WaitFpData() == ACK_TIMEOUT){
